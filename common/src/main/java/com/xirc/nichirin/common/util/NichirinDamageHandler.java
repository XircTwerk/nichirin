...
public final class NichirinDamageHandler {
    ...
    private static boolean shouldReduceArmorDamage(DamageSource source) {
        // Every mod attack type wears armor at the reduced rate — mod damage numbers are far
        // larger than vanilla hits, and at full rate they would shred the now-breakable armor.
        return REDUCE_ARMOR_DAMAGE.get() && NichirinDamageSources.isNichirinAttack(source);
    }
